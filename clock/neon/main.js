const STORAGE_KEY = "clockface-neon.settings.v1";
...
function drawHand(ctx, cx, cy, angle, length, width, color, alpha) {
  ctx.save();
  ctx.beginPath();
  ctx.moveTo(cx, cy);
...
function drawAnalogClock(canvas, settings) {
  ...
  // Draw hour markers with neon glow
  for (let i = 0; i < 60; i++) {
  ...
  drawHand(ctx, cx, cy, hrAngle,  R * 0.50, 6, textColor,   0.95);
  drawHand(ctx, cx, cy, minAngle, R * 0.72, 4, textColor,   0.88);
  ...
  if (settings.showSeconds) {
    ...
    ctx.moveTo(cx, cy);
    ctx.lineTo(
      cx + Math.cos(secAngle + Math.PI) * (R * 0.22),
      cy + Math.sin(secAngle + Math.PI) * (R * 0.22),
    );
    ...
    ctx.stroke();
    ctx.restore();
    drawHand(ctx, cx, cy, secAngle, R * 0.82, 3, accentColor, 1.0);
  }
